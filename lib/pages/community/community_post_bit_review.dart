...
class CommunityPostBitReview extends StatefulWidget {
  CommunityPostBitReview({
    Key key,
    this.data,
    this.isSecond = false,
    this.replyCall,
  ...
  @override
  State<CommunityPostBitReview> createState() => _CommunityPostBitReviewState();
}

class _CommunityPostBitReviewState extends State<CommunityPostBitReview> {
  dynamic _data;
  ...
  @override
  void didUpdateWidget(covariant CommunityPostBitReview oldWidget) {
    _data = widget.data;
    _comments = _data["comments"] == null ? [] : List.from(_data["comments"]);
    //回复超过5条就显示更多评论
    _tcoments = _comments.length > max ? _comments.sublist(0, max) : _comments;
            ...
            child: Row(
              ...
              children: [
                ...
                GestureDetector(
                  behavior: HitTestBehavior.translucent,
                  onTap: () {
                    bitTopicLike(type: "comment", id: _data["id"].toString())
                        .then((res) {
                      if (res.status == 1) {
