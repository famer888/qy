...
///最新界面
class ComicNewContent extends StatefulWidget {
  const ComicNewContent({super.key});

  @override
  State<ComicNewContent> createState() => _ComicNewContentState();
}

class _ComicNewContentState extends State<ComicNewContent> {
  late final _domain = context.read<ComicDomain>();

  Future<List<ComicItemsModel>?> _getData(
      {required int page, required int pageSize}) async {
    final result = await _domain.comicNewList(page: page, limit: pageSize);
