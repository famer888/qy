...
class NovelRecommendContent extends StatefulWidget {
  ...
  final int id;
  @override
  State<NovelRecommendContent> createState() => _NovelRecommendContentState();
}

class _NovelRecommendContentState extends State<NovelRecommendContent> {
  ...
  Future<List<RecommendNovelModel>?> _getData(int page, int pageSize) async {
    final result = await _domain.novelRecommend(
      page: page,
      limit: pageSize,
    );
    ...
    return Scaffold(
      body: MyListView.list(
        ...
        itemBuilder: (_, item, __) => item.map(
          ad: (data) => RecommendNovelAdCard(data: data),
          novel: (data) => RecommendNovelItemCard(data: data),
        ),
