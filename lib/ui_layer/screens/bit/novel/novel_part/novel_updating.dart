...
///连载界面
class NovelUpdatingScreen extends StatefulWidget {
  const NovelUpdatingScreen({super.key});

  @override
  State<NovelUpdatingScreen> createState() => _NovelUpdatingScreenState();
}

class _NovelUpdatingScreenState extends State<NovelUpdatingScreen> {
  late final _domain = context.read<NovelDomain>();

  Future<List<NovelItemModel>?> _getData(
      {required int page, required int pageSize}) async {
    final result = await _domain.novelUpdatingList(page: page, limit: pageSize);
